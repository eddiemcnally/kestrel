...
void
seatest_simple_test_result_log(int passed, char *reason,
		...
		if (seatest_verbose) {
			if (seatest_machine_readable) {
				printf("%s%s,%s,%u,Passed\r\n",
				       seatest_magic_marker,
				       seatest_current_fixture_path, function,
				       line);
			} else {
...
{
	seatest_simple_test_result(test, "Should have been true", function,
				   line);
...
{
	seatest_simple_test_result(!test, "Should have been false", function,
				   line);
}

void
seatest_assert_int_equal(int expected, int actual,
			 const char *function, unsigned int line)
{
...
void
seatest_assert_ulong_equal(unsigned long expected,
			   unsigned long actual, const char *function,
			   unsigned int line)
{
	char s[SEATEST_PRINT_BUFFER_SIZE];
...
void
seatest_assert_float_equal(float expected, float actual, float delta,
			   const char *function, unsigned int line)
{
	char s[SEATEST_PRINT_BUFFER_SIZE];
...
void
seatest_assert_double_equal(double expected, double actual,
			    double delta, const char *function,
			    unsigned int line)
{
	char s[SEATEST_PRINT_BUFFER_SIZE];
...
void
seatest_assert_string_equal(char *expected, char *actual,
			    const char *function, unsigned int line)
{
	int comparison;
	char s[SEATEST_PRINT_BUFFER_SIZE];
...
void
seatest_assert_string_ends_with(char *expected, char *actual,
				const char *function, unsigned int line)
{
	char s[SEATEST_PRINT_BUFFER_SIZE];
	sprintf(s, "Expected %s to end with %s", actual, expected);
	seatest_simple_test_result(strcmp
				   (expected,
...
void
seatest_assert_string_starts_with(char *expected, char *actual,
				  const char *function, unsigned int line)
{
	char s[SEATEST_PRINT_BUFFER_SIZE];
	sprintf(s, "Expected %s to start with %s", actual, expected);
	seatest_simple_test_result(strncmp(expected, actual, strlen(expected))
				   == 0, s, function, line);
}

void
seatest_assert_string_contains(char *expected, char *actual,
			       const char *function, unsigned int line)
{
	char s[SEATEST_PRINT_BUFFER_SIZE];
	sprintf(s, "Expected %s to be in %s", expected, actual);
	seatest_simple_test_result(strstr(actual, expected) != 0, s, function,
				   line);
}

void
seatest_assert_string_doesnt_contain(char *expected, char *actual,
				     const char *function, unsigned int line)
{
	char s[SEATEST_PRINT_BUFFER_SIZE];
...
int seatest_should_run(char *fixture, char *test)
{
	...
	if (seatest_fixture_filter) {
		if (strncmp
		    (seatest_fixture_filter, fixture,
		     strlen(seatest_fixture_filter)) != 0)
			run = 0;
	}
	if (seatest_test_filter && test != NULL) {
		if (strncmp
		    (seatest_test_filter, test, strlen(seatest_test_filter))
		    != 0)
			run = 0;
	}
...
int
seatest_parse_commandline_option_with_value(seatest_testrunner_t *
					    runner, int arg,
					    char *option,
...
void seatest_interpret_commandline(seatest_testrunner_t * runner)
{
	int arg;
	for (arg = 0;
	     (arg < runner->argc) && (runner->action != SEATEST_DO_ABORT);
	     arg++) {
		...
		if (seatest_parse_commandline_option_with_value
		    (runner, arg, "-t", test_filter))
			arg++;
		if (seatest_parse_commandline_option_with_value
		    (runner, arg, "-f", fixture_filter))
			arg++;
		if (seatest_parse_commandline_option_with_value
		    (runner, arg, "-k", set_magic_marker))
			arg++;
	}
}

void
seatest_testrunner_create(seatest_testrunner_t * runner, int argc, char **argv)
{
...
int
seatest_testrunner(int argc, char **argv, seatest_void_void tests,
...
void
seatest_simple_test_result_nolog(int passed, char *reason,
				 const char *function, unsigned int line)
{
